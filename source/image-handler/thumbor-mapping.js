...
class ThumborMapping {
   ...
   */
  parseCustomPath(path, queryStringParameters) {
    // Setup from the environment variables
    // Perform the substitution and return
    if (path !== undefined && queryStringParameters !== undefined) {
      var parsedPath = path;
      if (queryStringParameters.w && queryStringParameters.h) {
        parsedPath = `filters:stretch()/${queryStringParameters.w}x${queryStringParameters.h}/${parsedPath}`;
      }
